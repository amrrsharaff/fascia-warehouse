  ...
  public void setCount(File file, ArrayList<Fascia> fascias) {
    try {
      ...
      while (scanner.hasNextLine()) {
        ...
        for (Fascia fascia : fascias) {
          // If initial.csv has the amount for a fascia, have the count to be that amount.
    ...
    for (FasciaGroup group : pickedFascias) {
      if (!group.isSequenced()) {
        for (Order order : groups) {
          // if the order and the group of fascia have the same request ID
          if (group.getRequestId() == order.getRequestId()) {
            // have the sequencer compare the original order with the fascia picked by the picker
    ...
    try {
      ...
      while (scanner.hasNextLine()) {
        ...
        if (parts[0].equals("Order")) {
          ...
          if (parts[2].equals("ready")) { // declaring a ready picker
            ...
            for (Picker oldPicker : pickers) {
              if (oldPicker.getName().equals(parts[1])) {
                ...
                // nextGroup is used to find the next order within groups
          ...
          } else { // if they don't exist, introduce a new sequencer and assign the job to him
