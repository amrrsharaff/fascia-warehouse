...
public class Reader {
	 ...
	 * 
	 * @param file
	 *            the location of all the fascia.
	 * @param fascias
	 *            the list which will contain all the fascias.
	 */
	 ...
	 * 
	 * @param file
	 *            contains the location of each fascia.
	 * @param fascias
	 *            all the fascias in the system.
	 */
	 ...
	 * 
	 * @param file
	 *            contains the amount of those fascias that are less than 30 in
	 *            stock.
	 * @param fascias
	 *            all the fascias currently in the system.
	 */
	...
	/**
	 * The sequencer checks the fascias to make sure if they are correct. If
	 * not, the fascia are then corrected.
	 * 
	 * @param pickedFascias
	 *            the 8 fascias picked by the picker.
	 * @param groups
	 *            all the orders currently in the system.
	 * @param sequencer
	 *            the sequencer to sequence the pickedFascias.
	 * @param picker
	 *            the picker who picked the fascias.
	 */
	public void requestSequencer(ArrayList<FasciaGroup> pickedFascias, ArrayList<Order> groups, Sequencer sequencer,
			Picker picker) {
				...
				for (Order order : groups) {
					...
					if (group.getRequestId() == order.getRequestId()) {
						...
						if (!correct) {
							...
							FasciaGroup correctFascia = new FasciaGroup(order.getOrderFascia(), order.getRequestId());
							...
							System.out.println("The order with ID " + order.getRequestId() + " was picked correctly.");
						}
						// break;
					}
		...
		try {
			...
			while (scanner.hasNextLine()) {
				...
				if (parts[0].equals("Order")) {
					...
					if (parts[2].equals("ready")) { // declaring a ready picker
						...
						for (Picker oldPicker : pickers) {
							if (oldPicker.getName().equals(parts[1])) {
								...
								oldPicker.pickFascia(toBePicked, Integer.parseInt(parts[3]), fascias);
								if (oldPicker.isDone()) {
									FasciaGroup pickedGroup = new FasciaGroup(oldPicker.getFascias(),
											toBePicked.getRequestId());
									pickedFascias.add(pickedGroup);
									System.out.println(
											"All fascias in picking request number " + toBePicked.getRequestId()
					...
					// if sequencer exists: assign the job to him
					if (found) {
						for (Sequencer oldSequencer : sequencers) {
							if (oldSequencer.getName().equals(parts[1])) {
					...
					} else { // if they don't exist, introduce a new sequencer
								// and assign the job to him
						sequencer = new Sequencer(parts[1]);
						sequencers.add(sequencer);
					}

					if (parts[2].equals("sequences")) {
