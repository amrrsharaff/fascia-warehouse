...
public class Sequencer {
   ...
   * 
   * @param originalOrder
   *          the SKU numbers of the original 8 fascia requested within the
   *          order.
   * @param currentFascia
   *          the SKU numbers of the 8 fascia picked up by a picker.
   */
   ...
   * 
   * @param originalItem
   *          The original fascia that was ordered
   * @param currentItem
   *          the fascia to be compared
   */
  public boolean compare(Fascia originalItem, Fascia currentItem) {
    System.out.println("Sequencer " + getName() + " sequences.");
    if (!originalItem.getSku().equals(currentItem.getSku())) {
      System.out.println("Fascia with SKU " + originalItem.getSku() + " was incorrectly replaced with Fascia with SKU "
          + currentItem.getSku());
      return false;
    } else {
      return true;
